...
export default function CommentSection({ postId }: { postId: number }) {
    ...
    const getCurrentPageComments = () => {
        ...
        for (const comment of allComments) {
            ...
            if (currentCount >= page * COMMENTS_PER_PAGE && currentCount < (page + 1) * COMMENTS_PER_PAGE) {
                result.push(comment)
            }
    ...
    const renderComment = (comment: Comment, indent = 0) => (
        <div
            key={comment.id}
            className={`flex flex-col ${indent === 0 ? 'border-t border-gray-200 pt-4 pb-2' : 'ml-12 pb-4'}`}
        >
            ...
            {editingCommentId === comment.id ? (
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-2 justify-end">
                        <button onClick={() => setEditingCommentId(null)} className="text-gray-400 text-xs">
                            취소
                        </button>
                        <button
                            onClick={() => handleUpdate(comment.id)}
                            className="bg-[#78B3CE] text-white px-3 py-1 text-xs rounded hover:bg-[#5A8BA6]"
                        >
                            저장
                        </button>
                    </div>
    ...
    return (
        <div className="mt-10 border-t border-gray-200 pt-6 w-full max-w-4xl mx-auto">
            ...
            <div className="space-y-6 w-full">{comments.map((comment) => renderComment(comment))}</div>
                ...
                {Array.from({ length: totalPages }, (_, i) => (
                    <button
                        ...
                        className={`px-3 py-1 rounded ${
                            i === page ? 'bg-[#78B3CE] text-white' : 'bg-gray-100 text-gray-700'
                        }`}
                    >
                        {i + 1}
                    </button>
