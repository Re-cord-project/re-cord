...
export const useLatestPost = (userIdOrBlogname: number | string, initialAuthor?: AuthorDto | null) => {
    ...
    useEffect(() => {
        const fetchUserIdByBlogName = async () => {
            ...
            try {
                console.log(`API 호출 시작: ${process.env.NEXT_PUBLIC_API_URL}/api/auth/by-blogName/${blogName}`)
                ...
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/by-blogName/${blogName}`)
        ...
        const fetchLatestPost = async () => {
            ...
            try {
                ...
                if (!author) {
                    ...
                    const userResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/${userId}`, {
                        credentials: 'include',
                    })
                ...
                const postResponse = await fetch(
                    `${process.env.NEXT_PUBLIC_API_URL}/api/posts/public/latest/${userId}`,
                    {
                        credentials: 'include',
                    },
                )
