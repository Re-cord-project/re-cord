...
import Banner from '@/app/post/components/Banner'
...
const OtherBlogHomePage: React.FC = () => {
    // URL 쿼리에서 userId 파라미터 가져오기
    const searchParams = useSearchParams()
    const userIdParam = searchParams.get('userId')
    const userId = userIdParam ? parseInt(userIdParam, 10) : 0

    const { post, isLoading, error, author } = useLatestPost(userId)
                    ...
                    <div className="mb-4 relative overflow-hidden rounded-lg bg-gradient-to-r from-[#78B3CE] to-[#A8D5E5] p-0.5">
                        <div className="bg-white rounded-md p-5 flex flex-col md:flex-row items-center justify-between">
                            <div className="flex items-center mb-4 md:mb-0">
                                ...
                                <div>
                                    <p className="text-sm text-gray-500 font-medium">개발자의 기록</p>
                                    <h2 className="text-2xl font-bold text-gray-800">
                                        <span className="text-[#78B3CE]">{author.username}</span>님의 개발 블로그
                                    </h2>
                                </div>
                            </div>
