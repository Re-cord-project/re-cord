...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
    // 시큐리티에서 인증되지 않은 사용자가 보호된 리소스에 접근하려고 할 때 동작하는 인터페이스
    // 사용자가 인증되지 않았을 때 어떻게 응답할지를 정의해주기 위해 사용
    @Override
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException, ServletException {
        String exception = (String) request.getAttribute("exception");  // 필터에서 setAttribute로 넣어준 exception

        // rest와 page 요청에 대해 구분하여 응답하고 싶다!
    ...
    private void handleRestResponse(HttpServletRequest request, HttpServletResponse response, String exception) throws IOException {
        ...
        if (exception != null) {
            if (exception.equals(JwtExceptionCode.INVALID_TOKEN.getCode())) {
                ...
                log.error("entry point >> no exception code, unauthorized access");
    ...
    // page요청 중 예외 발생 시 로그 남기고 무조건 로그인폼으로 리다이렉트
    private void handlePageResponse(HttpServletRequest request, HttpServletResponse response, String exception) throws IOException {
        if (exception != null) {
            ...
            log.error("Page Request - No Exception Attribute, treating as unauthorized access.");
        }
